...
const PrivacyForm = () => {
    ...
    const fetchPrivacyDetails = async () => {
        setIsLoading(true);
        try {
            const res = await privacyService.getPrivacyById(id);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await privacyService.updatePrivacy(id, formData);
                toast.success('Privacy Policy updated successfully');
            } else {
                await privacyService.createPrivacy(formData);
                toast.success('New Privacy Policy created successfully');
            }
            navigate('/cms/privacy-policy');
        } catch (error) {
            ...
            <div className="stock-content-pane">
                <header className="stock-header-section" style={{ marginBottom: '2.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                        <button onClick={() => navigate('/cms/privacy-policy')} className="secondary-button" style={{ padding: '8px', width: '36px', height: '36px', borderRadius: '10px' }}>
                            <ArrowLeft size={18} />
                        </button>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            ...
                            <span style={{ fontSize: '0.7rem', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'hsl(var(--primary))' }}>
                                {isEdit ? 'Edit Privacy Policy' : 'Create New Policy'}
                            </span>
                        </div>
                    </div>
                    <div className="legal-header-flex">
                        <div style={{ maxWidth: '700px' }}>
                            <h1 style={{ fontSize: '2.5rem', fontWeight: '900', letterSpacing: '-0.03em', margin: 0 }}>
                                {isEdit ? `Editing: ${formData.title}` : 'Build Privacy Policy'}
                            </h1>
                            <p style={{ margin: '8px 0 0 0', color: 'hsl(var(--muted-foreground))', fontSize: '1.1rem' }}>
                                Draft the privacy policies and data protection agreements for your marketplace.
                            </p>
                        </div>
                ...
                <div className="legal-form-grid">
                    <div className="stock-glass-card" style={{ padding: '2rem', border: '1px solid hsl(var(--primary) / 0.1)', background: 'hsl(var(--card) / 0.4)', backdropFilter: 'blur(10px)' }}>
                        {previewMode ? (
                            ...
                            <form id="privacy-main-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                                <div className="stock-field-group">
                                    <label className="stock-label">
                                        <Type size={16} style={{ color: 'hsl(var(--primary))' }} /> Policy Title
                                    </label>
                                    <input
                                        type="text"
                                        ...
                                        placeholder="e.g. VS Mart Data Protection Policy"
                                        required
                                    />
                                </div>
                                ...
                                <div className="stock-field-group">
                                    ...
                                    <input
                                        ...
                                        placeholder="What does this policy version cover?"
                                    />
                                </div>
                                ...
                                <div className="stock-field-group">
                                    ...
                                    <div className="quill-editor-wrapper">
                                        <ReactQuill
                                            ...
                                            placeholder="Draft your privacy policy content here..."
                                            modules={{
                                                toolbar: [
                                                    [{ 'header': [1, 2, 3, false] }],
                        ...
                        <div className="stock-glass-card" style={{ padding: '1.5rem', border: '1px solid hsl(var(--border) / 0.5)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px', borderRadius: '14px', background: 'hsl(var(--primary) / 0.05)', border: '1px solid hsl(var(--primary) / 0.1)' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                    ...
                                    <span style={{ fontSize: '0.7rem', color: 'hsl(var(--muted-foreground))' }}>Make live for app users</span>
                                </div>
