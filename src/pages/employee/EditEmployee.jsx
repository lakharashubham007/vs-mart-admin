...
const EditEmployee = () => {
    ...
    const fetchInitialData = async () => {
        setIsLoading(true);
        try {
            const [rolesData, employee] = await Promise.all([
    ...
    return (
        <div className="employee-page-container fade-in">
            <div className="employee-content-pane employee-form-view">
                <header className="employee-header">
                    <div>
                        ...
                        <p>Modify account information and permissions for <strong>{formData.name}</strong>.</p>
                    </div>
                    <div style={{ display: 'flex', gap: '0.75rem' }}>
                        <button type="button" className="secondary-button" onClick={() => navigate(-1)}>Cancel</button>
                        <button type="submit" form="edit-employee-form" className="primary-button" disabled={isSaving}>
                            <Save size={18} /> {isSaving ? 'Saving...' : 'Update Employee'}
                        </button>
                    </div>
                </header>

                <div className="employee-glass-card">
                    <form id="edit-employee-form" className="employee-form-container" onSubmit={handleSubmit}>
            ...
            {(isLoading || isSaving) && <Loader message={isSaving ? "Updating Records..." : ""} />}
        </div>
    );
};
